...
public class ParallelGatewayTest extends AbstractFixFlowTestCase {
	...
	public void testForkToken() {
		...
		generalTaskCommand.setTaskId(taskInstanceQueryTo.getId());
		generalTaskCommand.setUserCommandId("general1");
		generalTaskCommand.setTaskComment("流程提交!");
		...
		//完成第一个分支任务
		generalTaskCommand = new GeneralTaskCommand();
		generalTaskCommand.setTaskId(taskInstanceQueryToList.get(0).getId());
		generalTaskCommand.setUserCommandId("general1");
		generalTaskCommand.setTaskComment("分支处理完毕!");
		taskService.commandComplete(generalTaskCommand);
		
		//完成第二个分支任务
		generalTaskCommand = new GeneralTaskCommand();
		generalTaskCommand.setTaskId(taskInstanceQueryToList.get(1).getId());
		generalTaskCommand.setUserCommandId("general1");
		generalTaskCommand.setTaskComment("分支处理完毕!");
		taskService.commandComplete(generalTaskCommand);
