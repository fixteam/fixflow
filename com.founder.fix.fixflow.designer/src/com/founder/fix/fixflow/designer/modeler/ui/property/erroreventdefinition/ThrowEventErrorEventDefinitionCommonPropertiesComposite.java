...
public class ThrowEventErrorEventDefinitionCommonPropertiesComposite extends
	...
	public ThrowEventErrorEventDefinitionCommonPropertiesComposite(
	...
	public void createUI() {
		...
		GridData gd_idtext = new GridData(SWT.LEFT, SWT.CENTER, false, false,
				1, 1);
		...
		GridData gd_descLabel = new GridData(SWT.FILL, SWT.TOP, false, false,
				1, 1);
		...
		GridData gd_desctext = new GridData(SWT.LEFT, SWT.CENTER, false, false,
				1, 1);
	...
	public void createUIBindings(EObject eObject) {
		...
		final ErrorEventDefinition errorEventDefinition = (ErrorEventDefinition) throwEvent
		...
		if (errorEventDefinition
				.eGet(FixFlowPackage.Literals.DOCUMENT_ROOT__ERROR_CODE) != null) {

			combo.setText(errorEventDefinition.eGet(
					FixFlowPackage.Literals.DOCUMENT_ROOT__ERROR_CODE)
					.toString());
					...
					editingDomain.getCommandStack().execute(
							new RecordingCommand(editingDomain) {
								...
								protected void doExecute() {
									...
									errorEventDefinition.getAnyAttribute()
											.clear();
							...
							editingDomain.getCommandStack().execute(
									new RecordingCommand(editingDomain) {
										...
										protected void doExecute() {
											...
											FeatureMap.Entry extensionAttributeEntry = new org.eclipse.emf.ecore.impl.EStructuralFeatureImpl.SimpleFeatureMapEntry(
													(org.eclipse.emf.ecore.EStructuralFeature.Internal) FixFlowPackage.Literals.DOCUMENT_ROOT__ERROR_CODE,
													error.getErrorCode());
											errorEventDefinition
													.getAnyAttribute().clear();
											errorEventDefinition
													.getAnyAttribute()
													.add(extensionAttributeEntry);
		...
		if (errorEventDefinition.getId() == null) {
			...
			editingDomain.getCommandStack().execute(
					new RecordingCommand(editingDomain) {
						@Override
						protected void doExecute() {
							errorEventDefinition.setId(ModelUtil.
									.generateID(errorEventDefinition));
						}
					});
		}*/
		...
		IObservableValue textObserver = SWTObservables.observeText(idtext,
				SWT.Modify);
				...
				if (!idtext.getText().equals(errorEventDefinition.getId())) {
					...
					editingDomain.getCommandStack().execute(
							new RecordingCommand(editingDomain) {
								@Override
								protected void doExecute() {
									errorEventDefinition.setId(e.diff
		...
		if (errorEventDefinition.getDocumentation().size() == 0
				|| errorEventDefinition.getDocumentation().get(0).getText() == null) {
			desctext.setText("");
		} else {
			desctext.setText(errorEventDefinition.getDocumentation().get(0)
					.getText());
		}

		IObservableValue textObserverDoc = SWTObservables.observeText(desctext,
				SWT.Modify);
				...
				editingDomain.getCommandStack().execute(
						new RecordingCommand(editingDomain) {
							...
							protected void doExecute() {
								...
								if (errorEventDefinition.getDocumentation()
										.size() == 0) {
									final Documentation value = Bpmn2Factory.eINSTANCE
											.createDocumentation();
									ModelUtil.setID(value, be.eResource());
									errorEventDefinition.getDocumentation()
											.add(value);
								}

								List<Documentation> documentationList = errorEventDefinition
										.getDocumentation();
								if (!desctext.getText().equals(
										documentationList.get(0).getText())) {

									documentationList.get(0).setText(
											e.diff.getNewValue().toString());
								}
							}
						});
