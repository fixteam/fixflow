 ...
 *
 */
public class CommonConnectorWizardPage extends WizardPage {
	 ...
	 * @param pageName
	 */
	public CommonConnectorWizardPage(String pageName) {
		super(pageName);
	 ...
	 * @param titleImage
	 */
	public CommonConnectorWizardPage(String pageName, String title, String desc,
			ImageDescriptor titleImage, Page page) {
...
//					connectNameText.setLayoutData(new GridData(GridData.FILL_HORIZONTAL)); //布局
	...
	private void dataChange() {
		...
		for (Iterator iterator = isRequiredControl.iterator(); iterator.hasNext();) {
			...
			else if(valcontrol instanceof ExpressionCombo) {
				ExpressionCombo expressionCombo = (ExpressionCombo) valcontrol;
				if(expressionCombo.getExpressionTo() == null || expressionCombo.getExpressionTo().getExpressionText().trim().equals("")) {
					setErrorMessage("请填写必填信息"); //提示错误信息
					setPageComplete(false); //使‘完成’和‘下一步’两个按钮不可用
					return;
				}
