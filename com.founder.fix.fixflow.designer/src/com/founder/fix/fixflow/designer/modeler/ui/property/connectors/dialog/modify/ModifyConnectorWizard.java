...
public class ModifyConnectorWizard extends Wizard {
	 ...
	 * 
	 */
	public ModifyConnectorWizard(EObject be, BPMN2Editor bpmn2Editor,
	...
	public void addPages() {
		//实例化向导页面
		modifyRenameConnectorWizardPage = new ModifyRenameConnectorWizardPage("modifyRenameConnectorWizardPage", "给连接器命名", 
				null, connectorInstance);
		...
		if(connector != null) {
			...
			if(pages != null && pages.size() > 0) {
				...
				for (int i = 0; i < pages.size(); i++) {
					...
					modifyCommonConnectotWizardPage = new ModifyCommonConnectorWizardPage("modifyCommonConnectotWizardPage", 
							page.getPageTitle(), page.getPageNote(), null, page, connectorInstance);
			...
			if(connector.getOutputs() != null) {
				...
				modifyOutputConnectorWizardPage = new ModifyOutputConnectorWizardPage("modifyOutputConnectorWizardPage",
						"输出配置", "映射连接器的输出到流程变量中", null, outputs, connectorInstance);
	...
	private void modifyConnectorInstance(final ConnectorInstance connectorInstance) {
		...
		TransactionalEditingDomain editingDomain = bpmn2Editor
				.getEditingDomain();
					...
					for (int j = 0; j < controls.size(); j++) {
						...
						if (control instanceof Label) {
							...
							//如果不是第一页，则得加上之前所有控件的数量再除，不然会更改掉之前页数相对应行的值
								...
								connectorParameterInputs = connectorInstance.getConnectorParameterInputs().get(((connectorInstance.getConnectorParameterInputs().size() * 2 - controls.size()) + j) / 2);
							}
