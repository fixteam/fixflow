...
public class ExpressionDialog extends Dialog {
	 ...
	 */
	public ExpressionDialog(Shell parentShell) {
		super(parentShell);
	 ...
	 * @param dataVarTo
	 */
	public ExpressionDialog(Shell parentShell, ExpressionTo expressionTo) {
		super(parentShell);
		setShellStyle(SWT.CLOSE | SWT.RESIZE | SWT.TITLE | SWT.PRIMARY_MODAL);
	...
	protected Control createDialogArea(Composite parent) {
		...
		gd_editorcomposite.widthHint = 450;
		editorcomposite.setLayoutData(gd_editorcomposite);

		text = new Text(editorcomposite, SWT.BORDER | SWT.WRAP | SWT.H_SCROLL | SWT.V_SCROLL | SWT.CANCEL);

		if (expressionTo != null && expressionTo.getExpressionText() != null)
			text.setText(expressionTo.getExpressionText());
		...
		// GroovyEditor备用
		/*
		 * IPreferenceStore store= new ChainedPreferenceStore(new
		 * IPreferenceStore[] { PreferenceConstants.getPreferenceStore(),
		 * EditorsUI.getPreferenceStore()});
		 * 
		 * FixGroovyEditor fge = new FixGroovyEditor(store); try {
		 * fge.init(PlatformUI
		 * .getWorkbench().getActiveWorkbenchWindow().getActivePage
		 * ().getActiveEditor().getEditorSite(), new StringInput("", "java")); }
		 * catch (PartInitException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); } fge.createPartControl(editorcomposite);
		 * fge.createJavaSourceViewerConfiguration();
		 */
		...
		functreeViewer.getTree().addMouseListener(new MouseListener() {
			...
			public void mouseDoubleClick(MouseEvent e) {
				...
				if (iStructuredSelection != null) {
					...
					if (offset == text.getText().length()) {
						sb.append(text.getText());
						sb.append(dataVariableDef.getValue());
					} else {
						String after = text.getText().substring(offset, text.getText().length());
						sb.append(before);
						sb.append(dataVariableDef.getValue());
					...
					if (dataVariableDef.getValue() != null) {
						text.setSelection(offset + dataVariableDef.getValue().length(), offset + dataVariableDef.getValue().length());
					}
				}
			}
		});
	...
	protected void okPressed() {
		...
		if(btnCheckButton.getSelection()){
			datavarname.setText(text.getText().replace(" ", ""));//把内容中空格去掉，不然存进id会报错
		}
		...
		if (expressionTo != null) {
			...
			expressionTo.setExpressionText(text.getText());
		}
...
//			String image1 = ISharedImages.IMG_OBJS_TASK_TSK;
	...
	public void createToolBar(Composite operatorcomposite) {
		...
		tltmNewItem.setText("+");
		tltmNewItem.addSelectionListener(new OperatorSelectionAdapter("+", text));

		ToolItem tltmNewItem_1 = new ToolItem(toolBar, SWT.NONE);
		tltmNewItem_1.setText("-");
		tltmNewItem_1.addSelectionListener(new OperatorSelectionAdapter("-", text));

		ToolItem tltmNewItem_2 = new ToolItem(toolBar, SWT.NONE);
		tltmNewItem_2.setText("*");
		tltmNewItem_2.addSelectionListener(new OperatorSelectionAdapter("*", text));

		ToolItem tltmNewItem_3 = new ToolItem(toolBar, SWT.NONE);
		tltmNewItem_3.setText("/");
		tltmNewItem_3.addSelectionListener(new OperatorSelectionAdapter("/", text));

		ToolItem tltmNewItem_4 = new ToolItem(toolBar, SWT.NONE);
		tltmNewItem_4.setText("<");
		tltmNewItem_4.addSelectionListener(new OperatorSelectionAdapter("<", text));

		ToolItem tltmNewItem_5 = new ToolItem(toolBar, SWT.NONE);
		tltmNewItem_5.setText("<=");
		tltmNewItem_5.addSelectionListener(new OperatorSelectionAdapter("<=", text));

		ToolItem tltmNewItem_6 = new ToolItem(toolBar, SWT.NONE);
		tltmNewItem_6.setText("==");
		tltmNewItem_6.addSelectionListener(new OperatorSelectionAdapter("==", text));

		ToolItem tltmNewItem_7 = new ToolItem(toolBar, SWT.NONE);
		tltmNewItem_7.setText("!=");
		tltmNewItem_7.addSelectionListener(new OperatorSelectionAdapter("!=", text));

		ToolItem tltmNewItem_8 = new ToolItem(toolBar, SWT.NONE);
		tltmNewItem_8.setText(">=");
		tltmNewItem_8.addSelectionListener(new OperatorSelectionAdapter(">=", text));

		ToolItem tltmNewItem_9 = new ToolItem(toolBar, SWT.NONE);
		tltmNewItem_9.setText(">");
		tltmNewItem_9.addSelectionListener(new OperatorSelectionAdapter(">", text));

		ToolItem tltmNewItem_10 = new ToolItem(toolBar, SWT.NONE);
		tltmNewItem_10.setText("And");
		tltmNewItem_10.addSelectionListener(new OperatorSelectionAdapter("&&", text));

		ToolItem tltmNewItem_11 = new ToolItem(toolBar, SWT.NONE);
		tltmNewItem_11.setText("Or");
		tltmNewItem_11.addSelectionListener(new OperatorSelectionAdapter("||", text));

		ToolItem tltmNewItem_12 = new ToolItem(toolBar, SWT.NONE);
		tltmNewItem_12.setText("Not");
		tltmNewItem_12.addSelectionListener(new OperatorSelectionAdapter("!", text));

		ToolItem tltmNewItem_13 = new ToolItem(toolBar, SWT.NONE);
		tltmNewItem_13.setText("true");
		tltmNewItem_13.addSelectionListener(new OperatorSelectionAdapter("true", text));

		ToolItem tltmNewItem_14 = new ToolItem(toolBar, SWT.NONE);
		tltmNewItem_14.setText("false");
		tltmNewItem_14.addSelectionListener(new OperatorSelectionAdapter("false", text));

		ToolItem tltmNewItem_15 = new ToolItem(toolBar, SWT.NONE);
		tltmNewItem_15.setText("(");
		tltmNewItem_15.addSelectionListener(new OperatorSelectionAdapter("(", text));

		ToolItem tltmNewItem_16 = new ToolItem(toolBar, SWT.NONE);
		tltmNewItem_16.setText(")");
		tltmNewItem_16.addSelectionListener(new OperatorSelectionAdapter(")", text));
...
//		dataVarTos.add(dataVarTo2);
		...
		//如果设置启用则添加所有用户
		if(FixFlowConfigUtil.getFixFlowConfig().getDesignerOrgConfig().getAllUserInfo().getIsEnabled().equals("true")) {
