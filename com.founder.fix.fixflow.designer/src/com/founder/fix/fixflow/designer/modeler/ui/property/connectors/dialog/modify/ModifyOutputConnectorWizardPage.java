 ...
 * 
 */
public class ModifyOutputConnectorWizardPage extends WizardPage {
	 ...
	 * @param pageName
	 */
	public ModifyOutputConnectorWizardPage(String pageName) {
		super(pageName);
	 ...
	 * @param titleImage
	 */
	public ModifyOutputConnectorWizardPage(String pageName, String title, String desc,
			ImageDescriptor titleImage, EList<OutputParameter> outputs, ConnectorInstance connectorInstance) {
	...
	public void createControl(Composite parent) {
		...
		//读取所有output参数
		EList<ConnectorParameterOutputs> parameterOutputs = connectorInstance.getConnectorParameterOutputs();
		
		if(parameterOutputs != null && parameterOutputs.size() > 0) {
			for (int i = 0; i < parameterOutputs.size(); i++) {
				final ConnectorParameterOutputs connectorParameterOutputs = parameterOutputs.get(i);
				...
				outputCombo.setText(connectorParameterOutputs
						.getExpression() == null ? ""
						: connectorParameterOutputs.getExpression().getValue());
...
//						dataChange();
				...
				outputCombo.addListener(SWT.Selection, new Listener() {
					...
					public void handleEvent(Event event) {
						...
						if(outputCombo.getSelectionIndex() == outputCombo.getItemCount()-1){
							ExpressionDialog expressionDialog = new ExpressionDialog(getShell(), connectorParameterOutputs
									.getExpression() == null ? ""
											: connectorParameterOutputs.getExpression().getValue());
				...
				targetCombo.setText(connectorParameterOutputs
						.getVariableTarget() == null ? ""
						: connectorParameterOutputs.getVariableTarget());
